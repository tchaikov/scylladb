...
get_sharder_for_tables(seastar::sharded<replica::database>& db, const sstring& keyspace, const std::vector<table_id>& table_ids) {
    ...
    if (!last_s) {
        throw std::runtime_error(seastar::format("Failed to find sharder for keyspace={}, tables={}, no table in this keyspace",
                keyspace, table_ids));
    }
...
future<int> repair_service::do_repair_start(sstring keyspace, std::unordered_map<sstring, sstring> options_map) {
    ...
    for (const auto& n: options.ignore_nodes) {
        try {
            ...
            throw std::runtime_error(seastar::format("Failed to parse node={} in ignore_nodes={} specified by user: {}",
        ...
        for (const auto& [keyspace_name, erm] : ks_erms) {
            ...
            for (auto&r : ranges) {
                ...
                if (new_owner.size() == 1) {
                    ...
                    throw std::runtime_error(seastar::format("{}: keyspace={}, range={}, current_replica_endpoints={}, new_replica_endpoints={}, wrong nubmer of new owner node={}",
