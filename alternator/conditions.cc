...
comparison_operator_type get_comparison_operator(const rjson::value& comparison_operator) {
    ...
    if (!comparison_operator.IsString()) {
        throw api_error::validation(seastar::format("Invalid comparison operator definition {}", rjson::print(comparison_operator)));
    }
    std::string op = comparison_operator.GetString();
    auto it = ops.find(op);
    if (it == ops.end()) {
        throw api_error::validation(seastar::format("Unsupported comparison operator {}", op));
    }
    return it->second;
}
...
static bool check_BETWEEN(const T& v, const T& lb, const T& ub, bool bounds_from_query) {
    if (cmp_lt()(ub, lb)) {
        if (bounds_from_query) {
            throw api_error::validation(
                seastar::format("BETWEEN operator requires lower_bound <= upper_bound, but {} > {}", lb, ub));
        } else {
            return false;
        }
    }
...
conditional_operator_type get_conditional_operator(const rjson::value& req) {
    ...
    if (s == "AND") {
        ...
        throw api_error::validation(
                seastar::format("'ConditionalOperator' parameter must be AND, OR or missing. Found {}.", s));
    }
}
