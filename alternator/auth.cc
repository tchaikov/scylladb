...
future<std::string> get_key_from_roles(service::storage_proxy& proxy, auth::service& as, std::string username) {
    ...
    if (!salted_hash_col || !can_login_col) {
        co_await coroutine::return_exception(api_error::unrecognized_client(seastar::format("Credentials cannot be fetched for: {}", username)));
    }
    ...
    if (result_set->empty()) {
        co_await coroutine::return_exception(api_error::unrecognized_client(seastar::format("User not found: {}", username)));
    }
    ...
    if (!salted_hash) {
        co_await coroutine::return_exception(api_error::unrecognized_client(seastar::format("No password found for user: {}", username)));
    }
