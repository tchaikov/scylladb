...
static big_decimal parse_and_validate_number(std::string_view s) {
    try {
        ...
        if (magnitude > 125) {
            throw api_error::validation(seastar::format("Number overflow: {}. Attempting to store a number with magnitude larger than supported range.", s));
        }
        if (magnitude < -130) {
            throw api_error::validation(seastar::format("Number underflow: {}. Attempting to store a number with magnitude lower than supported range.", s));
        }
        if (precision > 38) {
            throw api_error::validation(seastar::format("Number too precise: {}. Attempting to store a number with more significant digits than supported.", s));
        }
        return ret;
    } catch (const marshal_exception& e) {
        throw api_error::validation(seastar::format("The parameter cannot be converted to a numeric value: {}", s));
    }
...
bytes get_key_column_value(const rjson::value& item, const column_definition& column) {
    ...
    if (!key_typed_value) {
        throw api_error::validation(seastar::format("Key column {} not found", column_name));
    }
...
static const rjson::value& get_typed_value(const rjson::value& key_typed_value, std::string_view type_str, std::string_view name, std::string_view value_name) {
    if (!key_typed_value.IsObject() || key_typed_value.MemberCount() != 1 ||
        ...
        throw api_error::validation(
                seastar::format("Malformed value object for {} {}: {}",
    ...
    if (rjson::to_string_view(it->name) != type_str) {
        throw api_error::validation(
                seastar::format("Type mismatch: expected type {} for {} {}, got type {}",
...
big_decimal unwrap_number(const rjson::value& v, std::string_view diagnostic) {
    if (!v.IsObject() || v.MemberCount() != 1) {
        throw api_error::validation(seastar::format("{}: invalid number object", diagnostic));
    }
    auto it = v.MemberBegin();
    if (it->name != "N") {
        throw api_error::validation(seastar::format("{}: expected number, found type '{}'", diagnostic, it->name));
    }
    if (!it->value.IsString()) {
        // We shouldn't reach here. Callers normally validate their input
        // earlier with validate_value().
        throw api_error::validation(seastar::format("{}: improperly formatted number constant", diagnostic));
    }
...
rjson::value set_sum(const rjson::value& v1, const rjson::value& v2) {
    ...
    if (set1_type != set2_type) {
        throw api_error::validation(seastar::format("Mismatched set types: {} and {}", set1_type, set2_type));
    }
    if (!set1 || !set2) {
...
std::optional<rjson::value> set_diff(const rjson::value& v1, const rjson::value& v2) {
    ...
    auto [set2_type, set2] = unwrap_set(v2);
    if (set1_type != set2_type) {
        throw api_error::validation(seastar::format("Set DELETE type mismatch: {} and {}", set1_type, set2_type));
    }
    if (!set1 || !set2) {
