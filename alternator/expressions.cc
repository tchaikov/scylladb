...
static Result parse(const char* input_name, std::string_view input, Func&& f) {
    ...
    try {
        ...
        throw expressions_syntax_error(seastar::format("Failed parsing {} '{}': {}",
    ...
    if (column_name.size() > 0 && column_name.front() == '#') {
        if (!expression_attribute_names) {
            throw api_error::validation(
                    seastar::format("ExpressionAttributeNames missing, entry '{}' required by expression", column_name));
        }
    ...
    std::visit(overloaded_functor {
        [&] (const std::string& valref) {
            if (!expression_attribute_values) {
                throw api_error::validation(
                        seastar::format("ExpressionAttributeValues missing, entry '{}' required by expression", valref));
            }
            ...
            if (value->IsNull()) {
                throw api_error::validation(
                        seastar::format("ExpressionAttributeValues null value for entry '{}' required by expression", valref));
            }
    ...
    return std::visit(overloaded_functor {
        ...
        [&] (const parsed::value::function_call& f) -> rjson::value {
            ...
            if (function_it == function_handlers.end()) {
                throw api_error::validation(
                        seastar::format("{}: unknown function '{}' called.", caller, f._function_name));
            }
