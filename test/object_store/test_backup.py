...
def create_ks_and_cf(cql):
    ...
    cql.execute((f"CREATE KEYSPACE {ks} WITH"
                  f" REPLICATION = {replication_opts};"))
...
@pytest.mark.asyncio
async def test_simple_backup(manager: ManagerClient, s3_server, s3_client):
    ...
    objects = set([ o.key for o in s3_client.Bucket(s3_server.bucket_name).objects.all() ])
    for f in files:
