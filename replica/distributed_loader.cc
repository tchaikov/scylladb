...
distributed_loader::process_upload_dir(distributed<replica::database>& db, distributed<db::system_distributed_keyspace>& sys_dist_ks,
    ...
    return seastar::async(std::move(attr), [&db, &view_update_generator, &sys_dist_ks, ks = std::move(ks), cf = std::move(cf)] {
        ...
        std::vector<std::unique_ptr<sstables::sstable_generation_generator<sstables::is_shared::yes>>> gen;
        std::generate_n(std::back_inserter(gen),
                        ...
                        [last_generation = highest_generation_seen(directory).get0()] {
                            ...
                            return std::make_unique<sstables::sstable_generation_generator<sstables::is_shared::yes>>(base);
                        });
